...
>
> **The two hosts were not one deployment, and the cutover plan's assumption that they would follow production "for free" is false.** `viniciusoliveiras.vercel.app` is an attached project **domain** and does track production. `portfolio-viniciusoliveiras.vercel.app` was a manually pinned **alias created 2021-08-19**, and after the force-push it was still serving the 2021 Next.js site — `/pt` 404, `/home` 200, `age: 208089`. It had to be assigned to the new production deployment by hand. *(Amended the same day: once assigned, it does track production on subsequent deploys — verified on the next one. The repair is one-time, not recurring.)*

**And this table lists two origins where there are three.** `portfolio-git-main-viniciusoliveiras.vercel.app`, a legacy git-branch alias under Vercel's old naming scheme, was pinned to the same 2021 deployment and serving the same stale site. It carried `x-robots-tag: noindex`, so it never contributed to the duplicate-content defect this section identifies — but it is a third `*.vercel.app` host that has to be accounted for whenever the origins consolidate.
>
