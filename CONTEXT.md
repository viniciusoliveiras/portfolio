...
| **the mark grid / the label row** | The two shared grid shapes, bound as utilities because each holds an alignment across files that no single caller can see. `mark-grid` is the 260px mark column, drawn by `Section`'s split layout **and independently by every Experience row**; `label-row` is the 130px label column, which lines Skills' eight rows up with Education's two | a literal `grid-cols-[260px_1fr]` or `grid-cols-[130px_1fr]` at a call site. Those drifted between callers once already, which is why `page` exists | `src/styles/global.css` |
| **the bar** | The sticky top bar: the monogram at left, **five** anchors and the locale switch at right, translucent over the page. Below `wide` its anchors move into the sheet. Its height is `--spacing-bar`, **nominally 54px against a measured 50.39**, and the sheet's header, `scroll-margin-top` and Skills' sticky mark are all `calc()`s off it | *header*, *navbar* — and above all not *the mark*. **Six mark labels against five bar anchors is correct**, and the two are authored as separate string sets. Do not restate its height as a literal anywhere | [ADR-0006](docs/adr/0006-warm-editorial-direction.md), `src/portfolio/anchors.ts` |
...
| **on-accent** | Text on an accent fill: `#FFF8EF`, one value, because the accent is one value. Its only caller is `pill-solid`'s hover, the one place text sits on accent | a per-mode flip, which is what ADR-0005 needed. There was an `accent-surface` utility binding the pair; it had no caller and is gone | [ADR-0006 §Colour](docs/adr/0006-warm-editorial-direction.md) |
...
| **mark / meta / micro** | The three mono **label** roles, collapsed from the design's five near-identical variants. `mark` 11px/0.14em caps is the workhorse; `meta` 11px/0.10em is sentence case, for readable phrases rather than labels; `micro` 10px/0.14em caps is the hero meta labels and the footer | five separate variants, which is what the design file carries and what the token layer deliberately does not. And **not the whole of the mono layer** — see below | [ADR-0006 §The token layer](docs/adr/0006-warm-editorial-direction.md) |
| **caption / chip / pill** | The other three mono roles, at 12–13px, which never belonged to the label spread above: `caption` 12px/0.04em is the work cards' `·`-joined stack line, `chip` 13px the architecture chips, `pill` 12px/0.06em the contact links. **Six mono roles in total** | *the three mono roles* — that was true of the label collapse and never of the page. These three shipped as inline arbitrary values, which is the same thing with no name on it | [ADR-0006 §The token layer](docs/adr/0006-warm-editorial-direction.md) |
...
| **chrome** | Page furniture belonging to no single section. Two strings, both the footer's — `place` and `builtWith`. Not named `footer`, because the hero's eyebrow drew on it too until that was cut | *layout copy*, *boilerplate*. And **no availability claim belongs here**, or anywhere: `kind` and `availability` were deleted with the eyebrow, and a test now asserts the page makes none | [ADR-0006 §Consequences](docs/adr/0006-warm-editorial-direction.md), [site copy](docs/site-copy.md) |
